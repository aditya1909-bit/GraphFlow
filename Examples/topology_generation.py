"""
Topology generation demo: learn to reconstruct adjacency from node features.
"""
...
def edge_split(A, rng, val_frac=0.2):
    """Return train/val indices for undirected edges (upper triangle)."""
...
def get_params_state(params):
    return [p.data.copy() for p in params]


def set_params_state(params, state):
    for p, v in zip(params, state):
        p.data[:] = v


def sample_edge_batch(pos_idx, neg_idx, rng, n_pos=256):
...
def augment_features(A, X):
    ...
    # approximate clustering via 2-hop triangles count normalized by degree^2
    A2 = A @ A
...
def main():
    # Data
    A, X, _, _ = make_two_community_graph(n_per=60, p_in=0.15, p_out=0.02, seed=7)
    ...
    for ep in range(1, epochs + 1):
        ...
        for p in model.params:
            l2 = l2 + (p * p).sum()
        loss = loss + (l2 * l2_lambda)
        loss.backward()
        optimizer.step(model.params)

        if ep % 20 == 0:
            val_logits = model(X, pairs=val_pairs)
            val_loss = binary_cross_entropy_with_logits(val_logits, val_labels)
            acc = edge_accuracy(val_logits, val_labels)
            rec = edge_recall(val_logits, val_labels)
            spec = edge_specificity(val_logits, val_labels)
            print(
                f"Epoch {ep:03d} | loss={loss.data.item():.4f} | "
                f"val_loss={val_loss.data.item():.4f} | edge_acc={acc:.3f} | "
                f"recall={rec:.3f} | spec={spec:.3f}"
            )
            if val_loss.data.item() < best_val - 1e-4:
                best_val = val_loss.data.item()
                best_state = get_params_state(model.params)
                bad_epochs = 0
            else:
                bad_epochs += 1
                if bad_epochs >= patience:
                    print(f"Early stopping at epoch {ep:03d} | best_val_loss={best_val:.4f}")
                    break
    ...
    rec = edge_recall(val_logits, val_labels)
    spec = edge_specificity(val_logits, val_labels)
    print(f"Final val edge accuracy: {acc:.3f} | recall={rec:.3f} | spec={spec:.3f}")
